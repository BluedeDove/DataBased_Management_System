...
export class BookRepository {
  ...
  advancedSearch(criteria: {
    ...
    if (criteria.publisher) {
      ...
      params.push(`%${criteria.publisher}%`)
    }
    ...
    if (criteria.publish_date_start) {
      sql += ' AND b.publish_date >= ?'
      params.push(criteria.publish_date_start)
    }

    if (criteria.publish_date_end) {
      sql += ' AND b.publish_date <= ?'
      params.push(criteria.publish_date_end)
    }

    if (criteria.isbn) {
      sql += ' AND b.isbn LIKE ?'
      params.push(`%${criteria.isbn}%`)
    }
